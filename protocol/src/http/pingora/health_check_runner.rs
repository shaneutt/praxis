...
use super::health_check_probe::{http_probe, tcp_probe};
...
/// use praxis_protocol::http::pingora::health_check_runner::spawn_health_checks;
/// use tokio_util::sync::CancellationToken;
///
...
async fn probe_all_endpoints(params: &HealthCheckParams) {
    ...
    let futures: FuturesUnordered<_> = params
        ...
        .map(|(idx, addr)| {
            ...
            let expected_status = params.expected_status;
            let timeout = params.timeout;
            async move {
                let success = match check_type {
                    HealthCheckType::Http => http_probe(&addr, &path, expected_status, timeout).await,
                    HealthCheckType::Tcp => tcp_probe(&addr, timeout).await,
        ...
        if success {
            ...
            if params.state[idx].record_failure(params.unhealthy_threshold) {
                info!(
                    cluster = %params.cluster_name,
                    endpoint = %addr,
                    "endpoint transitioned to unhealthy"
                );
            }
        }
    }
