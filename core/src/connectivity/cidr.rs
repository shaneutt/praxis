...
impl CidrRange {
    ...
    pub fn contains(&self, ip: &IpAddr) -> bool {
        match (&self.addr, ip) {
            ...
            (IpAddr::V4(net), IpAddr::V6(candidate)) => {
                if let Some(mapped) = candidate.to_ipv4_mapped() {
                    let mask = v4_mask(self.prefix_len);
                    u32::from(*net) & mask == u32::from(mapped) & mask
                } else {
                    false
                }
            },
            (IpAddr::V6(net), IpAddr::V4(candidate)) => {
                if let Some(mapped) = net.to_ipv4_mapped()
                    && self.prefix_len >= 96
                {
                    let v4_prefix = self.prefix_len - 96;
                    let mask = v4_mask(v4_prefix);
                    u32::from(mapped) & mask == u32::from(*candidate) & mask
                } else {
                    false
                }
            },
