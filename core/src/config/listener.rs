...
pub struct Listener {
    ...
    ///
    /// Only applies to `protocol: http` listeners. Default: none
    /// (Falls back to Pingora's built-in 60s default).
    #[serde(default)]
...
"#;
        ...
        assert_eq!(tls.cert_path, "/certs/server.crt", "cert_path mismatch");
        assert_eq!(tls.key_path, "/certs/server.key", "key_path mismatch");
    }
