...
fn bench_production_pipeline_get() {
    ...
    // full pipeline adds ~7 filters of overhead vs baseline
...
fn bench_production_pipeline_post() {
    ...
    assert_eq!(result.errors, 0, "all requests should succeed");
    report_results(&result);
    // POST with JSON body through full pipeline
...
fn bench_production_pipeline_mixed() {
    ...
    // Alternate GET and POST within each worker thread.
    ...
    // mixed load through full pipeline; allow lower floor
    assert_performance(&result, 200.0, 500.0);
}
